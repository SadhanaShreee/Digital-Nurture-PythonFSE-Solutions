...
async def register(user: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    new_user = UserModel(email=user.email, hashed_password=hashed_password, is_active=1)

    db.add(new_user)
    await db.commit()
    await db.refresh(new_user)

    # Never return or log the plain-text password — only confirm success
...
async def create_course(course: CourseCreate, response: Response, current_user: UserModel = Depends(get_current_user) ,db: AsyncSession = Depends(get_db)):
...
async def delete_course(course_id: int, current_user: UserModel = Depends(get_current_user),db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(CourseModel).where(CourseModel.id == course_id))
    db_course = result.scalar_one_or_none()
